...
import org.alexdev.kepler.game.ig.*;
import org.alexdev.kepler.game.player.Player;
import org.alexdev.kepler.game.player.PlayerManager;
import org.alexdev.kepler.messages.outgoing.ig.ADD_BUDDY;
import org.alexdev.kepler.messages.outgoing.ig.BUDDY_REQUEST_RESULT;
...
public class MESSENGER_ACCEPTBUDDY implements MessageEvent {
    ...
    public void handle(Player player, NettyRequest reader) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            IGDao.newFriend(player.getDetails().getId(), userId);
            IGDao.newFriend(userId, player.getDetails().getId());
